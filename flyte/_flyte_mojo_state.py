...
def _dispatch(action, args, journal):
    \"\"\"An awaitable running `action` as a child action.\"\"\"
    task = _TASKS.get(action)
    if task is None:
        return mojo_action(action=action, args=args, journal=journal)
    return task(args=args, journal=journal)
"""
...
_ENV_BINDING = re.compile(r'comptime\s+(\w+)\s*=\s*TaskEnvironment\[\s*"([^"]+)"\s*\]')
_TASK_BINDING = re.compile(
    r'comptime\s+\w+\s*=\s*(\w+)\.task\[[^\]]*?name\s*=\s*"([^"]+)"\s*\]'
)
...
def remote_run_mojo(action, args, run_name=""):
